...
import { stopColor } from "@/lib/stop-colors";
    ...
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 sm:py-14">
      <section>
        <p className="font-mono text-[10.5px] tracking-[0.22em] text-accent uppercase">
          {stats.states.length} states so far · {trip.durationEstimate} · one continuous loop
        </p>
        <h1 className="mt-3 font-display text-4xl leading-[0.95] tracking-wide text-balance uppercase sm:text-6xl">
          {trip.tagline}
        </h1>
        <p className="mt-5 max-w-2xl text-[15px] leading-relaxed text-ink-soft sm:text-base">{trip.mission}</p>

        <div className="mt-7 flex flex-wrap gap-2.5">
          {trip.regions[0] && (
            <Link
              href={`/region/${trip.regions[0].id}/#day-1`}
        ...
        <p className="mt-6 max-w-2xl rounded-xl border border-hairline bg-surface p-4 text-sm">
          <span className="font-cond font-bold tracking-wide uppercase">Where to begin: </span>
      ...
      <section id="regions" className="mt-14 scroll-mt-20">
        <h2 className="font-display text-2xl tracking-wide uppercase sm:text-3xl">The route, region by region</h2>
        <p className="mt-2 max-w-2xl text-sm text-ink-soft">
          The loop runs clockwise and chases the seasons — each leg is timed for the weather that makes it worth being
          there.
        </p>

        <ol className="mt-5 space-y-3">
          {trip.regions.map((region, index) => {
            ...
            return (
              <li key={region.id}>
                ...
                >
                  <div className="flex">
                    <div aria-hidden className="w-2 shrink-0" style={{ backgroundColor: stopColor(index) }} />
                    <div className="min-w-0 flex-1 p-4 sm:p-5">
                      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                        <span className="font-mono text-[10px] tracking-[0.18em] text-muted uppercase">
                          Leg {region.order}
                        </span>
                        <h3 className="font-display text-xl tracking-wide uppercase">{region.name}</h3>
                      ...
                      <p className="mt-2 text-sm text-ink-soft">{region.summary}</p>
      ...
      <section className="mt-14">
        <h2 className="font-display text-2xl tracking-wide uppercase sm:text-3xl">Everything is color-coded</h2>
        <p className="mt-2 max-w-2xl text-sm text-ink-soft">
      ...
      <section className="mt-14 rounded-xl border border-hairline bg-surface p-5 sm:p-6">
        <h2 className="font-display text-xl tracking-wide uppercase">How to read the itinerary</h2>
        <dl className="mt-4 grid gap-4 sm:grid-cols-3">
          <div>
            <dt className="font-cond text-[15px] font-bold tracking-wide uppercase">Full days</dt>
            <dd className="mt-1 text-sm text-ink-soft">Planned hour by hour, from morning through the evening.</dd>
          </div>
          <div>
            <dt className="font-cond text-[15px] font-bold tracking-wide uppercase">Free days</dt>
            <dd className="mt-1 text-sm text-ink-soft">
              Empty on purpose. {stats.freeDays} of the {stats.days} days so far are yours.
            </dd>
          </div>
          <div>
            <dt className="font-cond text-[15px] font-bold tracking-wide uppercase">Drive days</dt>
            <dd className="mt-1 text-sm text-ink-soft">

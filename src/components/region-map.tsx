...
export function RegionMap({
  ...
  return (
    <div className="overflow-hidden rounded-xl border" style={{ borderColor: theme.landStroke, backgroundColor: theme.water }}>
      <svg
      ...
      >
        {map.states.map((state) => (
          <path key={state.id} d={state.d} fill={theme.land} stroke={theme.landStroke} strokeWidth={0.8} />
        ))}
        ...
        {map.stops.map((stop) => {
          ...
          const pin = (
            <>
              {active && <circle cx={stop.x} cy={stop.y} r={16} fill={color} opacity={0.25} />}
              <circle
                cx={stop.x}
                cy={stop.y}
                r={active ? 10 : 7}

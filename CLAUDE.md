...
- **The map is build-time SVG.** `src/lib/geo.ts` projects each region's own stops with d3-geo, crops to them, and decimates outlines to whole pixels. `regionMapData` draws one leg; `nationalMapData` draws every written leg on one projection for the home page. Three traps are documented in that file and all are easy to reintroduce: Albers folds far-away geometry into view (filter states geographically *before* projecting), a polygon ring with the wrong winding makes `fitExtent` fit the whole globe (fit to MultiPoint corners instead), and a lat/lng box is the wrong thing to fit the *nation* to — its corners sit in two oceans, so fit to the land itself. Render it and look at it (`scripts/dev/map-preview.ts`, `scripts/dev/nation-preview.ts`) after touching that file.
- **Driving and resting are events, not day-level footnotes.** A block holds exactly one of `item`, `drive`, or `rest`, so a day can open with a three-hour drive and still have an afternoon in town. `src/lib/day-timeline.ts` flattens all of it — including the older day-level `commute` object and bare `free` days — into one ordered list, so the renderer only ever sees one kind of event. Whatever is *optional* (a free day's menu, the stops worth pulling over for) sits below the timeline, so a day's commitments and its suggestions never blur together. This is also what uses the `commuting` and `free-rest` categories: no authored item can carry them, but the renderer assigns them to drive and rest blocks.
- **End times are computed** (`src/lib/schedule.ts`) from the authored start plus `durationMins` when content doesn't give one, so every event shows a span without anyone maintaining two clocks per item. A block with only a loose `slot` starts at the *later* of where the previous block ended and what its label implies — so "Evening" can neither overlap the afternoon nor resolve to half past three. An event with no start, end or duration reads "All day" only when nothing precedes it (a whole free day); after something else it reads "from 6:00 PM", because that's what the rest of a day is.
- **Share cards are the one committed asset.** `scripts/build-og.ts` (`npm run og`) draws the Open Graph images from the same projection code the site uses and rasterises them with sharp into `public/og/`. They are generated on demand and **committed**, not built, because sharp resolves SVG text against the *machine's* fonts and the build machine is Cloudflare's — building them in CI would make the site's most public image depend on a font stack nobody can see. `npm run validate` warns when the numbers drawn on them no longer match the content. Every page that declares `openGraph` must name its image explicitly: Next replaces a parent's `openGraph` object wholesale when a child defines one, so an image set only in the root layout disappears from every inner page.
...
- **Search and answer engines are a first-class output.** `src/lib/seo.ts` composes canonical URLs and descriptions; `src/lib/schema-org.ts` emits JSON-LD (`TouristTrip` for the trip and each leg, `TouristDestination` per stop, an ordered `ItemList` of `TouristAttraction` per day, `BreadcrumbList` everywhere), rendered server-side by `src/components/json-ld.tsx`. `app/sitemap.ts` and `app/robots.ts` are emitted as static files by the export, and `scripts/build-llms-txt.ts` writes `public/llms.txt` (gitignored — regenerated by `npm run build`). All of it derives from the content, so none of it can describe an itinerary that doesn't exist. **Never assert something in structured data that the page doesn't show** — no ratings, prices, or dates.

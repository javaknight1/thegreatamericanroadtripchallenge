...
- **End times are computed** (`src/lib/schedule.ts`) from the authored start plus `durationMins` when content doesn't give one, so every event shows a span without anyone maintaining two clocks per item. A block with only a loose `slot` is placed after the block before it.

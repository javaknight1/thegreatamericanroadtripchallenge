...
- **Every day type is a timeline.** A free day gets a scheduled "rest — the day is yours" block and a drive day gets its hours on the clock, because both consume a day just as surely as a full one does. Whatever is *optional* — a free day's menu, the stops worth pulling over for — sits in a separate section below the timeline, so the day's commitments and its suggestions never blur together. This is also what finally uses the `commuting` and `free-rest` categories: no authored item can carry them, but the renderer assigns them to the blocks it synthesises.
